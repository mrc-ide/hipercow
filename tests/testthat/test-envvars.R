test_that("empty envvars is s3 object still", {
  e <- envvars()
  expect_s3_class(e, c("envvars", "data.frame"), exact = TRUE)
...
test_that("can collect environment variables", {
  ...
  expect_equal(envvars(A = "x", b = "y", secret = TRUE),
...
test_that("environment variables must be scalars", {
  expect_error(
    envvars(A = "x", B = 1:2),
    "All arguments to 'envvars' must be scalars")
})


test_that("coerce to character", {
  expect_equal(envvars(A = "x", B = 1)$value, c("x", "1"))
})
...
test_that("can concatenate environment variables", {
  e1 <- envvars(A = "1")
  e2 <- envvars(X = "x", Y = "y")
  e3 <- envvars(M = "m", N = "n", O = "o", secret = TRUE)
  ...
  expect_error(c(e1, "E" = 1),
               "Can't combine 'envvars' objects and other objects")
})
