...
Furthermore, if you are referring to the cluster-storage within a cluster job, you should refer to `wpia-hn` as `wpia-hn-app` to ensure you use the fast Infiniband network connection to get your data. For example, if I have some data that I refer to from my W`indows local computer as `//wpia-hn.hpc.dide.ic.ac.uk/flu-project/data.dat`, then to access the same data from a cluster job, I should write `//wpia-hn-app/flu-project/data.dat`.
