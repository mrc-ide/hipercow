...
test_that("can get a task status", {
  ...
  path_root <- root$path$root
  config <- root$config[["dide-windows"]]
  expect_equal(dide_status(id, config, path_root), "submitted")

  file.create(path_to_task_file(path_root, id, "status-running"))
  expect_equal(dide_status(id, config, path_root), "running")
})
...
test_that("can get a task result", {
  ...
  expect_silent(dide_result(id, config, path_root))
  expect_equal(
...
test_that("can cancel a task", {
  ...
  mockery::stub(dide_cancel, "get_web_client", mock_get_client)

  res1 <- dide_cancel(id, config, path_root)
  ...
  res2 <- dide_cancel(id, config, path_root)
...
test_that("can cancel a bunch of tasks, in reverse order", {
  ...
  expect_equal(dide_cancel(ids, config, path_root)$cancelled,
               c(TRUE, TRUE, FALSE))
...
test_that("can read a task log", {
  ...
  expect_null(dide_log(id, FALSE, config, path_root))
  file.create(path_log)
  expect_equal(dide_log(id, FALSE, config, path_root), character())
  writeLines(c("a", "b", "c"), path_log)
  expect_equal(dide_log(id, FALSE, config, path_root), c("a", "b", "c"))
})
...
test_that("can read dide log", {
  ...
  mock_get_client <- mockery::mock(mock_client)
  mockery::stub(dide_log, "get_web_client", mock_get_client)
  ...
  expect_equal(dide_log(id, TRUE, config, path_root),
               c("some", "logs"))
...
test_that("can get task info", {
  ...
  mock_get_client <- mockery::mock(mock_client)
  mockery::stub(dide_info, "get_web_client", mock_get_client)
  ...
  res <- dide_info(id, config, path_root)
...
test_that("can check hello and switch to fast queue for windows", {
  ...
  mockery::stub(windows_check_hello, "dide_check", mock_check)
...
test_that("can check hello and switch to fast queue for linux", {
  ...
  mock_check <- mockery::mock(TRUE)
  mockery::stub(linux_check_hello, "dide_check", mock_check)
