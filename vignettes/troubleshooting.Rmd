...
* run `task_info()` with your id which will fetch this true id and tell you about any discrepency
* checking the web portal to see if the task is really listed as queued.  Check the failed tasks and see if it's there
* look at the *outer* logs (`task_log_show(id, outer = TRUE)`) which will show you the schduler's logs for this task, which may be informative.
