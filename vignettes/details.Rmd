...
* driver-specific variables (defined by the `default_envvars` argument to `hipercow_driver`); see `vignette("dide-cluster")` for defaults specific to our DIED cluster
...
Not every minor version are installed, and available versions may differ between `dide-windows` and `dide-linux`. But we should have a version close enough to what you need.
