...
The underlying reason for this is that we store information about the state of tasks your network share; if you run out of space while writing the final result of a task that task will remain as `running` indefinitely because we fail to write out that it succeeded.
