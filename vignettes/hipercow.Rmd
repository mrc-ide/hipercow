...
By *platform*, we mean operating system. You can use Hipercow on your own computer, whether it is a Windows, Linux or Mac. There are differences between these operating systems, mostly about how to refer to your DIDE network paths, as
they appear on your computer (ie, drive letters or mountpoints), and Hipercow tries to handle those for you without any fuss.

Until recently, your jobs would then be run on Windows nodes. However, unusually (and perhaps interestingly), our DIDE cluster runs Microsoft HPC, which can have both linux-based and windows-based nodes attached to it, using the same basic launch tools for either. This is interesting, since it can allow linux jobs to be run, but with a launch mechanism extremely similar to what you might already be familiar with from Hipercow.
