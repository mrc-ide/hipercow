...
Typically, you will use a different driver here (e.g., `hipercow_init(driver = "windows")`) or initialise without setting up a driver and then add one later by running `hipercow_configure()`.
...
Here, you can see versions of important packages, information about where you are working, and information about how you intend to interact with the cluster (for the Windows cluster this will include the cluster name, the mapping if the current directory into something the cluster understands, and the version of R that tasks will run on when running on the cluster).  If you have issues with `hipercow` we will always want to see the output of `hipercow_configuration()`.
...
Because we are using the "example" driver here, this is the same as the result that you'd get running `sessionInfo()` directly, just with more steps.  See `vignette("hello")` for an example that runs on Windows.
...
Likewise, the information about the task itself is wihtin `$data`. To work with the underling data you might just unclass the object to see the structure:
