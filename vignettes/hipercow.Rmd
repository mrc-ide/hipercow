...
* You can have multiple environments and each task can be set to run in a diferent environment
...
For example, here's a task that will sleep for 30 seconds, which we submit to the cluster:

```{r}
id <- task_create_expr(Sys.sleep(30))
```

```{r, include = FALSE}
task_wait(id, for_start = TRUE)
```
...
You can cancel a task that is submitted (waiting to be picked up by a cluster) or running.  You can cancel many tasks at once by passing a vector of identifiers at the same time.  Tasks that have finished (successfully or not) cannot be cancelled.
