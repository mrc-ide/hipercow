...
This iterates over the data in a **row-wise** way.  Note that this is very different to `lapply` which would iterate over **columns** (in practice we find that this almost never what people want).  The names of the columns must match the names of your function arguments and all columns must be used.
...
b <- hipercow_bundle_create(c(id1, id2), "my_new_budle")
b
```
