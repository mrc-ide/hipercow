...
* There is no guarantee that the object you see when creating the task is the same as the object loaded by `sources`.  We can add validation if you would find that useful
...
hipercow_task_create(use_pkg_a())
hipercow_environment_create(packages = "pkg.b")
hipercow_task_create(use_pkg_b())
```
...
hipercow_task_create(use_pkg_a(), environment = "env_a")
hipercow_environment_create(packages = "pkg.b", name = "env_a")
hipercow_task_create(use_pkg_b(), environment = "env_b")
```
