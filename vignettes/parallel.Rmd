...
Requesting a number of cores through `hipercow_resoures` alone will not make any difference to the behaviour or performance of your code; it only affects the resources the cluster reserves and allocates to your task, as it decides what tasks to run on which nodes. To make use of the cores we reserved, we need to use `hipercow_parallel`, which at present supports two methods for running different code on the different cores you have reserved. One is the `parallel` package, and the other is the `future` package.

## Using the Parallel package

In this example, we reserve 2 cores on the cluster, and then call `hipercow_parallel("parallel")` which sets up a team of workers (2 in this case), each of which use one of the allocated cores.
...
The `future` package, is similar in use to `parallel`. In testing, it appears slower than `parallel`.
...
In this example, we would need the `furrr` package to be provisioned using `hipercow_provision`.

## How many cores does each process have?

The number of cores available to a process can be looked up with `hipercow_parallel_get_cores`. For the top-level parent process, this will be the same as the number of cores requested using `hipercow_resources`, but for the workers created by initialising the `future` or `parallel` clusters, the result will be 1, because there will be a separate process per core.
