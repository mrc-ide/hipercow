...
Some packages (such as Dust or Stan) will can use these environment variables and run in parallel without you having to do anything further. There are also ways you can explicitly say how many threads you would like to use - see below.
...
task_wait(id)
task_result(id)[, c("expression", "total_time")]
```
...
```{r res_mem}
resources <- hipercow_resources(memory_per_node = "256G", exclusive = TRUE, cores = 8)
```
