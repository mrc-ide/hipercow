...
We support one cluster at present (the "windows" cluster) but will support more in future; in particular we aim to support a Linux cluster within DIDE ("hermod") and the central ICT/RCS cluster.  In future we may support others, but we have immediate plans to.
...
#> ℹ Configuration for 'windows' unchanged
windows_check()
...
#> ✔ DIDE credentials are correct
#> ✔ Connection to private network working
#> ✔ Submitted task '1322354ea44fde0e87aabc7bb622eb97' using 'windows'
#> 
#> ── hipercow 0.2.25 running at 'V:/cluster/hipercow-vignette/hv-20240125-188a0231
#> ℹ library paths:
#> •
#> V:/cluster/hipercow-vignette/hv-20240125-188a0231af4698/hipercow/lib/windows/4.3.2
#> • I:/bootstrap/4.3.2
#> • C:/Program Files/R/R-4.3.2/library
#> ℹ id: 1322354ea44fde0e87aabc7bb622eb97
#> ℹ starting at: 2024-01-25 17:38:11.240695
#> ℹ task type: expression
#> ℹ expression: { [...]
#> ℹ exporting 1 local variable into the execution environment: 'moo'
#> ───────────────────────────────────────────────────────────────── task logs ↓ ──
#>  -----
#> Moooooo!
#>  ------
...
#> ───────────────────────────────────────────────────────────────── task logs ↑ ──
#> ✔ status: success
#> ℹ finishing at: 2024-01-25 17:38:11.240695 (elapsed: 0.5772 secs)
#> ✔ Successfully ran test task '1322354ea44fde0e87aabc7bb622eb97'
```
...
#> 
#> ── hipercow root at /home/rfitzjoh/net/home/cluster/hipercow-vignette/hv-20240125-188a0231af4698 ────────
#> ✔ Working directory '.' within root
#> ℹ R version 4.3.2 on Linux (rfitzjoh@wpia-dide300)
#> 
#> ── Packages ──
#> 
#> ℹ This is hipercow 0.2.31
#> ℹ Installed: hipercow.windows (0.2.31), conan2 (1.9.97), logwatch (0.1.0)
#> 
#> ── Environments ──
#> 
#> ── default
...
#> • r_version: 4.3.2
#> • path_lib: hipercow/lib/windows/4.3.2
#> • username: rfitzjoh
```

<!-- copied over from troubleshooting; not sure if we want this; better might be to cook this up as a mini bit and include into the main troubleshooting doc? -->

# Troubleshooting 

Here, we simulate one so you can see how to troubleshoot it.  I'm going to *deliberately* misconfigure the network share that this is running on so that the cluster will not be able to map it and the task will fail to start
...
#> ✔ Submitted task '2390686c70b634626a4e0c9908e0f26c' using 'windows'
```
...
#> Error in `task_wait()`:
#> ! Task '2390686c70b634626a4e0c9908e0f26c' did not complete in time (status: timeout)
```
...
If you suspect this has happened, you can check the DIDE web interface, or you can run `task_info()` which does a more detailed check on the task than `task_status()` does:


```r
task_info(id)
#> ! Fixing status of '2390686c70b634626a4e0c9908e0f26c' from 'submitted' to 'failure'
#> ℹ If this is unexpected, consider checking the logs for more information:
#> • task_logs("2390686c70b634626a4e0c9908e0f26c")
#> • task_logs("2390686c70b634626a4e0c9908e0f26c", outer = TRUE)
#> 
#> ── task 2390686c70b634626a4e0c9908e0f26c (failure) ──────────────────────────────────────────────────────
#> ℹ Submitted with 'windows'
#> ℹ Task type: expression
#>   • Expression: sessionInfo()
#>   • Locals: (none)
#>   • Environment: default
#> ℹ Created at 2024-01-25 17:38:13.135672 (less than a minute ago)
#> ✖ Start time unknown!
#> ℹ Finished at 2024-01-25 17:38:24.25529 (moments ago; ran for ???)
```

Here, `task_info()` has worked out what has happened and suggested some places to go hunting for more information; first the task log:


```r
task_log_show(id)
#> ✖ No logs for task '2390686c70b634626a4e0c9908e0f26c' (yet?)
```

and secondly the "outer log" which is the logging from the cluster scheduler itself, before it hands off to our task:


```r
task_log_show(id, outer = TRUE)
#> Task failed during execution with exit code . Please check task's output for error details.
#> Output                          : The network path was not found.
```

On the windows cluster this can be informative, but with some luck it will very rarely contain anything you need to see.
