...
**The user interface is quite different, with free functions replacing the big `queue_didehpc` object**.  Previously you created an object (often `obj`) with `didehpc::queue_didehpc` and then interact with this by running a method (say `obj$task_result()`); instead in `hipercow` you use a free function (in this example `task_result`). See below for a translation of methods to new function(s).
...
* `login`: `windows_check` (we don't actualy log in this way though)
* `mapply`: TODO (mrc-4895)
* `provision_context`: `hipercow_provision`
* `reconcile`: `task_info` (but only one id at a time, and and only reconciles as a side effect)
* `rrq_controller`: 
...
Previously you might have used `didehpc_config` to change lots of bits of didehpc's behaviour. We've removed most of this for now!  Below is a list of arguments to [`didehpc::didehpc_config()`](https://mrc-ide.github.io/didehpc/reference/didehpc_config.html) and 
...
* `temp`: this used to be information about temp drive, we do not mount this by default either.
...
* `use_java`: Java is not currently supported
* `java_home`: Java is not currently supported
