...
**The user interface is quite different, with free functions replacing the big `queue_didehpc` object**.  Previously you created an object (often `obj`) with [`queue_didehpc`](https://mrc-ide.github.io/didehpc/reference/queue_didehpc.html) and then interact with this by running a method (say `obj$task_result()`); instead in `hipercow` you use a free function (in this example `task_result`). See below for a translation of methods to new function(s).
...
* `use_java`: Java is not currently supported, but will once supported will no longer be configurable
* `java_home`: Java is not currently supported, but will once supported will no longer be configurable
