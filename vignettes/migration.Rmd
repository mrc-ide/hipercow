...
* `template`: this is renamed to `queue` in `hipercow_resources()`, which is what `template` essentially meant. You then pass your resources list when creating tasks.
...
* `wholenode`: this is more flexible now. Setting `cores` to `Inf` will request a whole node, how ever many cores it has. Additionally, setting `exclusive` to `TRUE` ensures that no other task runs on the node at the same time as yours, even if you request a smaller number of cores than the node has.
* `parallel`: call `hipercow_parallel` with the argument `"future"` or `"parallel`" and pass the result as the `parallel` argument when creating your task, to use multiple cores. This improves in clarity and implementation on what `didehpc` meant by `parallel`.
...
* Dependencies between tasks (e.g., submit a task after another has completed)
